...
class MaterializeFixtureRepoTest(unittest.TestCase):
    ...
    def test_creates_real_repo_with_tracked_and_untracked_changes(self) -> None:
    ...
    def test_writes_mock_lpr_state_inside_git_directory(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            repo = Path(temp) / "repo"
    ...
    def test_refuses_to_write_into_non_empty_destination(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            repo = Path(temp) / "repo"
            repo.mkdir()
